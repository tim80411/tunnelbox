...
- 確認目標開發伺服器已啟動且正在監聽指定的 Port
- 確認代理目標 URL 正確（包含 `http://` 前綴和正確的 Port）
- 若目標伺服器重啟，TunnelBox 的代理會自動重新連線

### WebSocket 連線中斷

- Proxy 模式支援 WebSocket 轉發，確認目標伺服器的 WebSocket 正常
- 若使用 Vite 等工具的 HMR，確認防火牆未阻擋 WebSocket 連線

## LAN 分享

### 無法顯示 LAN URL

- 確認裝置已連接區域網路（Wi-Fi 或有線）
- VPN 介面會被自動過濾，如需透過 VPN 分享需另行配置
- 若有多個網路介面，TunnelBox 會自動選擇主要介面
...
- 確認網路連線正常
- 前往 [GitHub Releases](https://github.com/tim80411/tunnelbox/releases) 手動下載最新版
